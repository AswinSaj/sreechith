  ...
  featureFilms: [
    {
      ...
      description:
        "A compelling drama that explores the depths of human emotion and cultural identity.",
    ...
    },
    {
      ...
      description:
        "An inspiring journey through the challenges of medical education.",
      category: "Feature Films",
      type: "video" as const,
      buttons: [
...
export default function Works() {
  ...
  const {
    data: featureFilms,
    loading: featureFilmsLoading,
    error: featureFilmsError,
  } = useSanityData<FeatureFilm[]>(FEATURED_FEATURE_FILMS_QUERY);
  const {
    data: webSeries,
    loading: webSeriesLoading,
    error: webSeriesError,
  } = useSanityData<WebSeries[]>(WEB_SERIES_QUERY);
  const {
    data: shortOriginalFilms,
    loading: shortOriginalFilmsLoading,
    error: shortOriginalFilmsError,
  } = useSanityData<ShortOriginalFilm[]>(SHORT_ORIGINAL_FILMS_QUERY);
  const {
    data: digitalCommercials,
    loading: digitalCommercialsLoading,
    error: digitalCommercialsError,
  } = useSanityData<DigitalCommercial[]>(DIGITAL_COMMERCIALS_QUERY);
  const {
    data: digitalFilms,
    loading: digitalFilmsLoading,
    error: digitalFilmsError,
  } = useSanityData<DigitalFilm[]>(DIGITAL_FILMS_QUERY);
  const {
    data: documentaryFilms,
    loading: documentaryFilmsLoading,
    error: documentaryFilmsError,
  } = useSanityData<DocumentaryFilm[]>(DOCUMENTARY_FILMS_QUERY);
  const {
    data: digitalCorporateFilms,
    loading: digitalCorporateFilmsLoading,
    error: digitalCorporateFilmsError,
  } = useSanityData<DigitalCorporateFilm[]>(DIGITAL_CORPORATE_FILMS_QUERY);
  const {
    data: musicFilms,
    loading: musicFilmsLoading,
    error: musicFilmsError,
  } = useSanityData<MusicFilm[]>(MUSIC_FILMS_QUERY);
  const {
    data: filmPromoDigitalFilms,
    loading: filmPromoDigitalFilmsLoading,
    error: filmPromoDigitalFilmsError,
  } = useSanityData<FilmPromoDigital[]>(FILM_PROMO_DIGITAL_QUERY);
  const {
    data: upcomingReleases,
    loading: upcomingReleasesLoading,
    error: upcomingReleasesError,
  } = useSanityData<UpcomingRelease[]>(UPCOMING_RELEASES_QUERY);
  ...
  const createVideoItem = (
    ...
    return {
      ...
      thumbnail: item.thumbnail?.asset?._ref
        ? urlFor(item.thumbnail as { asset: { _ref: string } })
            .width(400)
            .height(600)
            .fit("crop")
            .url()
  ...
  const featureFilmsData = useMemo(() => {
    ...
    if (featureFilms && featureFilms.length > 0) {
      return featureFilms.map((film) => {
        ...
        const thumbnailUrl =
          film.thumbnail && film.thumbnail.asset?._ref
      ...
      >
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
            ...
            {item.description && (
              <p
                className="text-gray-300 text-xs mt-1 overflow-hidden text-ellipsis"
                style={{
                  display: "-webkit-box",
                  WebkitLineClamp: 2,
  ...
  if (
    featureFilmsLoading ||
    webSeriesLoading ||
    shortOriginalFilmsLoading ||
    digitalCommercialsLoading ||
    digitalFilmsLoading ||
    documentaryFilmsLoading ||
    digitalCorporateFilmsLoading ||
    musicFilmsLoading ||
    filmPromoDigitalFilmsLoading ||
          ...
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <p className="text-gray-400 text-lg max-w-2xl">
                A journey through films, series, commercials, and creative
                storytelling
              </p>
            </motion.div>
  ...
  if (
    featureFilmsError ||
    webSeriesError ||
    shortOriginalFilmsError ||
    digitalCommercialsError ||
    digitalFilmsError ||
    documentaryFilmsError ||
    digitalCorporateFilmsError ||
    musicFilmsError ||
    filmPromoDigitalFilmsError ||
        ...
        >
          ...
          <div className="w-full overflow-hidden">
            {featureFilmsData.length > 0 ? (
              <Carousel
                items={featureFilmsData.map((card, cardIndex) => (
                  <Card
          ...
          <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-col space-y-16">
                ...
                >
                  ...
                  <div className="space-y-4">
                    <ExpandableCardStandard
                      items={
                        shortOriginalFilms && shortOriginalFilms.length > 0
                          ? shortOriginalFilms
                          : fallbackData.shortOriginals.map((item) => ({
                              _id: item.id,
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {(digitalCommercials && digitalCommercials.length > 0
                      ? digitalCommercials
                      : fallbackData.digitalCommercials.map((item) => ({
                          _id: item.id,
                          title: item.title,
                          thumbnail: {
                            _type: "image" as const,
                      ...
                      const videoItem = createVideoItem(
                        item,
                        fallbackData.digitalCommercials[index]?.thumbnail
                      );
                      return (
                        <VideoCard
                          key={videoItem.id}
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {(documentaryFilms && documentaryFilms.length > 0
                      ? documentaryFilms
                      : fallbackData.documentaries.map((item) => ({
                          _id: item.id,
                          title: item.title,
                          thumbnail: {
                            _type: "image" as const,
                      ...
                      const videoItem = createVideoItem(
                        item,
                        fallbackData.documentaries[index]?.thumbnail
                      );
                      return (
                        <VideoCard
                          key={videoItem.id}
              ...
              <div className="flex flex-col space-y-16">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {(webSeries && webSeries.length > 0
                      ? webSeries
                      : fallbackData.webseries.map((item) => ({
                          _id: item.id,
                          title: item.title,
                          thumbnail: {
                            _type: "image" as const,
                      ...
                      const videoItem = createVideoItem(
                        item,
                        fallbackData.webseries[index]?.thumbnail
                      );
                      return (
                        <VideoCard
                          key={videoItem.id}
                ...
                >
                  ...
                  <div className="space-y-4">
                    <ExpandableCardGrid
                      items={
                        filmPromoDigitalFilms &&
                        filmPromoDigitalFilms.length > 0
                          ? filmPromoDigitalFilms
                          : fallbackData.filmPromos.map((item) => ({
                              _id: item.id,
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {(digitalCorporateFilms && digitalCorporateFilms.length > 0
                      ? digitalCorporateFilms
                      : fallbackData.corporateFilms.map((item) => ({
                          _id: item.id,
                          title: item.title,
                          thumbnail: {
                            _type: "image" as const,
                      ...
                      const videoItem = createVideoItem(
                        item,
                        fallbackData.corporateFilms[index]?.thumbnail
                      );
                      return (
                        <VideoCard
                          key={videoItem.id}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {(digitalFilms && digitalFilms.length > 0
                  ? digitalFilms
                  : fallbackData.webseries.map((item) => ({
                      _id: item.id,
                      title: item.title,
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {(musicFilms && musicFilms.length > 0
                  ? musicFilms
                  : fallbackData.corporateFilms.map((item) => ({
                      _id: item.id,
                      title: item.title,
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                {(upcomingReleases && upcomingReleases.length > 0
                  ? upcomingReleases
                  : fallbackData.upcomingProjects.map((item) => ({
                      _id: item.id,
                      title: item.title,
                      thumbnail: {
                        _type: "image" as const,
                  ...
                  const videoItem = createVideoItem(
                    item,
                    fallbackData.upcomingProjects[index]?.thumbnail
                  );
                  return (
                    <VideoCard
                      key={videoItem.id}
            ...
            <div className="text-center mt-16">
              <motion.button
                ...
                onClick={() => {
                  ...
                  const showreelUrl =
                    showreelItem?.videoUrl ||
                    fallbackData.corporateFilms.find((item) =>
                      item.title.toLowerCase().includes("showreel")
                    )?.videoUrl ||
                    "https://www.youtube.com/watch?v=Ek4g8TUXwZ8";

                  openVideoModal({
                    url: showreelUrl,
                    title: "Cinematography Showreel",
                    description:
                      "A compilation of my best cinematography work showcasing various projects and styles.",
                    category: "Showreel",
                  });
                }}
              >
